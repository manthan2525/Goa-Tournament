...
const TeamDetailsModal = ({ registration, onClose, onReviewPayment, onReviewAadhaar }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col my-auto rounded-3xl bg-white border border-slate-200 shadow-2xl p-5 sm:p-7 space-y-5 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 pb-3 flex-shrink-0">
          <div>
            ...
            <h3 className="font-display font-bold text-xl text-slate-900 mt-1">{registration.teamName}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-5 overflow-y-auto pr-1 flex-1">
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase font-mono ${
                registration.status === 'VERIFIED' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                registration.status === 'REJECTED' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-yellow-50 text-yellow-800 border border-yellow-200'
              }`}>
                {registration.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-200 text-xs">
              <div>
                <span className="text-slate-500 block text-[10px] uppercase font-semibold mb-0.5">Email Address</span>
                <span className="text-slate-900 font-medium break-all">{email}</span>
              </div>
              ...
              {!isSamePhoneWhatsapp && (
                <div>
                  <span className="text-slate-500 block text-[10px] uppercase font-semibold mb-0.5">WhatsApp Number</span>
                  <span className="text-emerald-700 font-mono font-medium">{whatsapp}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200">
              {email !== 'N/A' && (
                <a
                  href={`mailto:${email}`}
                  className="px-3 py-1.5 rounded-xl bg-white hover:bg-slate-100 text-slate-700 text-xs font-bold flex items-center gap-1.5 border border-slate-300 transition-colors shadow-xs"
                >
              ...
              {(whatsapp !== 'N/A' || phone !== 'N/A') && (
                <a
                  ...
                  className="px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 text-xs font-bold flex items-center gap-1.5 border border-emerald-200 transition-colors shadow-xs"
                >
          ...
          <div className="space-y-2">
            ...
            {registration.playersList && registration.playersList.length > 0 ? (
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden text-xs shadow-xs">
                <div className="grid grid-cols-12 bg-slate-100 p-2.5 font-bold text-slate-600 text-[10px] uppercase border-b border-slate-200">
                ...
                <div className="divide-y divide-slate-100 max-h-40 overflow-y-auto">
                  {registration.playersList.map((p, i) => (
                    <div key={i} className="grid grid-cols-12 p-2.5 items-center text-slate-800">
                      <div className="col-span-1 font-mono text-slate-400">{i + 1}</div>
                      <div className="col-span-6 font-semibold text-slate-900 truncate">{p.name}</div>
                      <div className="col-span-3 text-slate-600">{p.role || 'Player'}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5">
              ...
              <div className="flex items-center justify-between">
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase font-mono ${
                  registration.paymentStatus === 'VERIFIED' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                  registration.paymentStatus === 'REJECTED' ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-yellow-50 text-yellow-800 border border-yellow-200'
                }`}>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5">
              <span className="text-slate-500 font-semibold block text-[10px] uppercase">Aadhaar Status</span>
              <div className="flex items-center justify-between">
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase font-mono ${
                  registration.aadhaarVerificationStatus === 'VERIFIED' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                  registration.aadhaarVerificationStatus === 'REJECTED' ? 'bg-rose-50 text-rose-700 border border-rose-200' :
                  registration.aadhaarVerificationStatus === 'NOT_REQUIRED' ? 'bg-slate-100 text-slate-600 border border-slate-300' : 'bg-teal-50 text-teal-700 border border-teal-200'
                }`}>

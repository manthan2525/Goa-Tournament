      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] flex flex-col my-auto rounded-2xl glass-panel border border-slate-700 shadow-2xl p-4 sm:p-7 space-y-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-display font-bold text-lg sm:text-xl text-white">Aadhaar Document Verification</h3>
              <p className="text-xs text-slate-400 truncate max-w-[240px] sm:max-w-md">
                Team: <span className="text-emerald-400 font-semibold">{registration.teamName}</span>
              </p>
            </div>
          </div>
          <button
              ...
              {registration.aadhaarDocument ? (
                isPdf ? (
                  <div className="p-6 text-center space-y-3">
                    ...
                    <p className="text-xs text-slate-300 font-semibold">PDF Document Uploaded</p>
                    <a
                      href={registration.aadhaarDocument}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 text-teal-300 text-xs font-bold hover:bg-slate-700 transition-colors"
                    >
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3.5 sm:p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-2.5">
              <div>
                ...
                <p className="font-semibold text-white text-sm">{registration.captainName}</p>
                <p className="text-slate-400 font-mono">{registration.contactPhone}</p>
              </div>
            ...
            {rejecting ? (
              ...
              <div className="space-y-2 pt-1">
                <button
                  ...
                  className="w-full py-3 min-h-[44px] rounded-xl text-xs font-bold uppercase tracking-wider bg-teal-500 hover:bg-teal-400 text-slate-950 flex items-center justify-center gap-2 shadow-lg shadow-teal-500/20 disabled:opacity-50 transition-all"
                >

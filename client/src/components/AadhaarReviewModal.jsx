      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] flex flex-col my-auto rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-4 sm:p-7 space-y-4 overflow-hidden text-slate-900 dark:text-white">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2">
            <div className="p-2 rounded-xl bg-teal-50 dark:bg-teal-950/40 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-800/50">
              <FileText className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-display font-bold text-lg sm:text-xl text-slate-900 dark:text-white">Aadhaar Document Verification</h3>
              <p className="text-xs text-slate-600 dark:text-slate-400 truncate max-w-[240px] sm:max-w-md">
                Team: <span className="text-emerald-700 dark:text-emerald-400 font-bold">{registration.teamName}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 items-start overflow-y-auto pr-1 flex-1">
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 aspect-video sm:aspect-square flex items-center justify-center group relative max-h-60 sm:max-h-none">
              {registration.aadhaarDocument ? (
                isPdf ? (
                  <div className="p-6 text-center space-y-3">
                    <FileText className="w-12 h-12 text-teal-600 dark:text-teal-400 mx-auto" />
                    <p className="text-xs text-slate-700 dark:text-slate-300 font-semibold">PDF Document Uploaded</p>
                    <a
                      href={registration.aadhaarDocument}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-teal-700 dark:text-teal-300 text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors shadow-xs"
                    >
                  ...
                  <>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="absolute top-2 right-2 p-2 rounded-xl bg-white/90 dark:bg-slate-900/90 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700 shadow-xs transition-opacity"
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3.5 sm:p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-2.5">
              <div>
                <p className="text-slate-500 dark:text-slate-400 font-medium">Captain / Player In-Charge</p>
                <p className="font-semibold text-slate-900 dark:text-white text-sm">{registration.captainName}</p>
                <p className="text-slate-600 dark:text-slate-400 font-mono">{registration.contactPhone}</p>
              </div>

              <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                <p className="text-slate-500 dark:text-slate-400 font-medium">Aadhaar Status</p>
                <span
                  className={`inline-block mt-1 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider text-[10px] border ${
                    registration.aadhaarVerificationStatus === 'VERIFIED'
                      ...
                      : 'bg-yellow-50 dark:bg-yellow-950/40 text-yellow-800 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800/50'
                  }`}
                >
              ...
              {registration.aadhaarRejectionReason && (
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 text-rose-700 dark:text-rose-400">
                  <p className="text-slate-500 dark:text-slate-400 font-medium">Previous Rejection Reason</p>
            ...
            {rejecting ? (
              <form onSubmit={handleReject} className="space-y-3 pt-1">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-2.5 bg-white dark:bg-slate-900 border border-rose-300 dark:border-rose-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-rose-500"
                  ></textarea>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-2.5 min-h-[44px] text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center border border-slate-200 dark:border-slate-700"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <div className="space-y-2 pt-1">
                ...
                <button
                  ...
                  className="w-full py-2.5 min-h-[44px] text-xs font-semibold text-rose-700 dark:text-rose-400 hover:text-rose-800 dark:hover:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40 border border-rose-200 dark:border-rose-800/50 rounded-xl transition-colors flex items-center justify-center"
                >
                  Reject with Reason
                </button>
              </div>

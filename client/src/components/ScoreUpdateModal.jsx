  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
        {[{ label: "Team A", score: scoreA, setScore: setScoreA, side: "A" },
          ...
          <div key={side} className="text-center space-y-2">
            ...
            <div className="text-3xl sm:text-4xl font-mono font-black text-emerald-700">{score}</div>
            <div className="flex items-center justify-center gap-2">
              <button type="button" onClick={() => setScore((p) => Math.max(0, p - 1))}
                className="w-10 h-10 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-700 flex items-center justify-center transition-all active:scale-95">
            ...
            <button type="button" onClick={() => addGoal(side)}
              className="w-full py-1.5 rounded-lg text-[10px] font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 transition-colors">
              ⚽ + Goal
            </button>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Match Minute</label>
          <input type="number" min="0" max="120" value={fd.minute}
            onChange={(e) => setFd({ minute: Math.max(0, parseInt(e.target.value) || 0) })}
            className="w-full min-h-[44px] px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 font-mono focus:outline-none focus:border-emerald-500" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Half</label>
          <div className="grid grid-cols-2 gap-1.5">
            {[1, 2].map((h) => (
              <button key={h} type="button" onClick={() => setFd({ half: h })}
                className={`min-h-[44px] rounded-xl text-xs font-bold border transition-all ${fd.half === h ? "bg-emerald-50 border-emerald-300 text-emerald-800 font-bold" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`}>
                {h === 1 ? "1st" : "2nd"}
              </button>
            ))}
          </div>
      ...
      {fd.goals.length > 0 && (
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-1.5">
          <p className="text-[10px] uppercase font-bold text-slate-500 mb-2">Goals</p>
          {fd.goals.map((g, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-slate-700">
              <span>⚽</span>
              <span className="font-mono font-bold text-emerald-700">{g.minute}&apos;</span>
              <span className="font-semibold flex-1">{g.team}</span>
              <button type="button" onClick={() => setFd({ goals: fd.goals.filter((_, j) => j !== i) })}
                className="text-rose-600 hover:text-rose-700 text-[10px] font-bold">✕</button>
            </div>
          ))}
        </div>
      )}
...
const CricketControls = ({ scoreA, setScoreA, liveData, setLiveData }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={key} className="col-span-1">
              <label className="block text-[10px] font-semibold text-slate-700 mb-1">{label}</label>
              <input type="number" min={min} max={max} value={value}
                onChange={(e) => setCd({ [key]: Math.max(min ?? 0, parseInt(e.target.value) || 0) })}
                className="w-full min-h-[44px] px-2 py-2 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 font-mono text-center focus:outline-none focus:border-emerald-500" />
            </div>
          ))}
          <div>
            <label className="block text-[10px] font-semibold text-slate-700 mb-1">Overs</label>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[1, 2, 4, 6].map((n) => (
            <button key={n} type="button" onClick={() => addRuns(n)}
              className={`py-2 rounded-xl text-xs font-black border transition-all active:scale-95 ${n === 6 ? "bg-emerald-50 border-emerald-300 text-emerald-800 hover:bg-emerald-100" : n === 4 ? "bg-teal-50 border-teal-300 text-teal-800 hover:bg-teal-100" : "bg-white border-slate-300 text-slate-800 hover:bg-slate-100"}`}>
              +{n}
            </button>
          ))}
        </div>
        <button type="button" onClick={addWicket}
          className="w-full py-2 rounded-xl text-xs font-black bg-rose-50 border border-rose-200 text-rose-700 hover:bg-rose-100 transition-colors active:scale-95 disabled:opacity-40"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Innings</label>
          <div className="grid grid-cols-2 gap-1.5">
            {[1, 2].map((inn) => (
              <button key={inn} type="button" onClick={() => setCd({ innings: inn })}
                className={`min-h-[44px] rounded-xl text-xs font-bold border transition-all ${cd.innings === inn ? "bg-emerald-50 border-emerald-300 text-emerald-800" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`}>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Target (2nd Inn)</label>
      ...
      {/* Batting team */}
      <div>
        <label className="block text-xs font-semibold text-slate-700 mb-1">Batting Team</label>
        <div className="grid grid-cols-2 gap-1.5">
          {[{ val: 1, label: "Team A" }, { val: 2, label: "Team B" }].map(({ val, label }) => (
            <button key={val} type="button" onClick={() => setCd({ battingTeam: val })}
              className={`min-h-[44px] rounded-xl text-xs font-bold border transition-all ${cd.battingTeam === val ? "bg-emerald-50 border-emerald-300 text-emerald-800" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`}>
              {label}
            </button>
          ))}
        </div>
...
const BadmintonControls = ({ scoreA, setScoreA, scoreB, setScoreB, liveData, setLiveData }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Current set points */}
      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 space-y-3">
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Set {bd.currentSet || 1} — Current Points</p>
        <div className="grid grid-cols-2 gap-3">
          {[{ label: "Team A", score: bd.pointA || 0, side: "A" }, { label: "Team B", score: bd.pointB || 0, side: "B" }].map(({ label, score, side }) => (
            <div key={side} className="text-center space-y-2">
              <p className="text-xs font-bold text-slate-900">{label}</p>
              <div className={`text-3xl font-black font-mono ${side === "A" ? "text-emerald-700" : "text-teal-700"}`}>{score}</div>
              <div className="flex items-center justify-center gap-2">
        ...
        <button type="button" onClick={finalizeSet}
          className="w-full py-2 rounded-xl text-xs font-black bg-amber-50 border border-amber-200 text-amber-800 hover:bg-amber-100 transition-colors">
      ...
      {(bd.sets || []).length > 0 && (
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-1.5">
          <p className="text-[10px] uppercase font-bold text-slate-500 mb-2">Completed Sets</p>
          {bd.sets.map((s, i) => (
            <div key={i} className="flex items-center justify-between text-xs text-slate-700">
              <span className="text-slate-500">Set {i + 1}</span>
              <span className={`font-mono font-bold ${s[0] > s[1] ? "text-emerald-700" : "text-teal-700"}`}>{s[0]} – {s[1]}</span>
...
const GenericControls = ({ scoreA, setScoreA, scoreB, setScoreB }) => (
  <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
    {[{ label: "Team A", score: scoreA, setScore: setScoreA }, { label: "Team B", score: scoreB, setScore: setScoreB }].map(({ label, score, setScore }, i) => (
      <div key={i} className={`text-center space-y-2 ${i === 1 ? "border-l border-slate-200 pl-3" : ""}`}>
        <p className="text-xs font-bold text-slate-900">{label}</p>
        <div className="text-3xl sm:text-4xl font-mono font-black text-emerald-700">{score}</div>
        <div className="flex items-center justify-center gap-2">
          <button type="button" onClick={() => setScore((p) => Math.max(0, p - 1))}
            className="w-10 h-10 rounded-xl bg-slate-200 hover:bg-slate-300 active:scale-95 text-slate-700 flex items-center justify-center transition-all">
            <Minus className="w-4 h-4" />
          </button>
          <button type="button" onClick={() => setScore((p) => p + 1)}
...
const ScoreUpdateModal = ({ match, onClose, onUpdated, onEditDetails, sport: sportProp }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-lg max-h-[92vh] flex flex-col my-auto rounded-2xl bg-white border border-slate-200 shadow-2xl p-4 sm:p-7 space-y-4 sm:space-y-5 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 pb-3 flex-shrink-0">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-lg sm:text-xl text-slate-900">{sport} Scorepad</h3>
            </div>
            <p className="text-xs text-slate-500 font-medium">Match #{match.matchNumber} • {match.round}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 overflow-y-auto pr-1 flex-1">
          ...
          {/* Match Status */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Match Status</label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { val: "SCHEDULED", label: "Scheduled", color: "border-slate-300 text-slate-700" },
                { val: "LIVE", label: "🔴 LIVE", color: "border-rose-300 bg-rose-50 text-rose-700 font-bold" },
                { val: "COMPLETED", label: "✓ Final", color: "border-emerald-300 bg-emerald-50 text-emerald-800 font-bold" },
              ].map((item) => (
                <button key={item.val} type="button" onClick={() => setStatus(item.val)}
                  className={`min-h-[44px] py-2 px-1.5 rounded-xl text-xs font-bold border transition-all flex items-center justify-center ${status === item.val ? `${item.color} ring-2 ring-emerald-500/40` : "border-slate-200 bg-white text-slate-600 hover:border-slate-300"}`}>
                  {item.label}
                </button>
              ))}
            </div>
          ...
          {/* Commentary */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Live Commentary</label>
            <input type="text" placeholder="e.g. 78' Spectacular goal by Rohit!" value={summary}
              onChange={(e) => setSummary(e.target.value)}
              className="w-full min-h-[44px] px-3.5 py-2 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-emerald-500" />
          </div>

          {/* Pitch / Court */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Pitch / Court / Arena</label>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-200 flex-shrink-0">
            {onEditDetails ? (
              <button type="button" onClick={() => { onClose(); onEditDetails(match); }}
                className="px-3.5 py-2.5 min-h-[44px] text-xs font-bold text-teal-700 bg-teal-50 hover:bg-teal-100 border border-teal-200 rounded-xl transition-colors">
                Edit Fixture →
              </button>
            ) : <div />}
            <div className="flex items-center gap-2">
              <button type="button" onClick={onClose}
                className="px-4 py-2.5 min-h-[44px] text-xs font-semibold text-slate-600 hover:text-slate-900 rounded-xl transition-colors">
                Cancel
              </button>

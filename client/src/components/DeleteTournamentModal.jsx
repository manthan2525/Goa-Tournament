...
const DeleteTournamentModal = ({ tournament, onClose, onDeleted }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-md my-auto rounded-2xl glass-panel border border-rose-500/30 shadow-2xl p-5 sm:p-7 space-y-4 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2 text-rose-400">
            ...
            <div>
              <h3 className="font-display font-bold text-lg text-white">Delete Tournament</h3>
              <p className="text-[11px] text-slate-400 truncate max-w-[220px]">{tournament.name}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleDelete} className="space-y-4 text-xs">
          <div>
            ...
            <input
              ...
              className="w-full min-h-[44px] px-3.5 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white text-sm font-mono focus:border-rose-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
            ...
            <button
              ...
              className="px-5 py-2.5 min-h-[44px] rounded-xl text-xs font-bold uppercase tracking-wider bg-rose-600 hover:bg-rose-500 text-white shadow-lg shadow-rose-600/20 disabled:opacity-40 transition-all flex items-center justify-center gap-1.5"
            >

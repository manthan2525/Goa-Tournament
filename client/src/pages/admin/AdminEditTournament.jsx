  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="h-96 rounded-3xl bg-slate-800/50 animate-pulse" />
      </div>
    );
  }

  if (!form) return <div className="text-rose-400 text-sm p-4">{error || 'Tournament not found.'}</div>;

  const fieldClass = "w-full px-3.5 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-sm text-white focus:border-amber-500 focus:outline-none";
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display font-black text-2xl text-white">Edit Tournament</h1>
          <p className="text-xs text-slate-400">Admin override — changes apply to any organizer's tournament</p>
        </div>
      </div>

      {error && <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex gap-2"><ShieldAlert className="w-4 h-4 flex-shrink-0" />{error}</div>}
      {success && <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300 flex gap-2"><ShieldCheck className="w-4 h-4 flex-shrink-0" />{success}</div>}

      <form onSubmit={handleSubmit} className="rounded-3xl glass-panel border border-slate-800 p-6 sm:p-8 space-y-8">
        {/* Basic Info */}
        <section className="space-y-4">
          <h3 className="font-bold text-amber-400 text-sm border-b border-slate-800 pb-2">Tournament Info</h3>
        ...
        {/* Venue & Schedule */}
        <section className="space-y-4">
          <h3 className="font-bold text-amber-400 text-sm border-b border-slate-800 pb-2">Venue & Schedule</h3>
        ...
        {/* Teams & Fee */}
        <section className="space-y-4">
          <h3 className="font-bold text-amber-400 text-sm border-b border-slate-800 pb-2">Teams & Payment</h3>
        ...
        {/* Winners */}
        <section className="space-y-4">
          <h3 className="font-bold text-amber-400 text-sm border-b border-slate-800 pb-2">Winners (if completed)</h3>
        ...
        {/* Rules & Identity */}
        <section className="space-y-4">
          <h3 className="font-bold text-amber-400 text-sm border-b border-slate-800 pb-2">Rules & Verification</h3>
          <div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <div>
              ...
              <p className="text-[11px] text-slate-400">Players must upload an ID document when registering</p>
            </div>
            <button
              type="button"
              onClick={() => setForm(prev => ({ ...prev, requireAadhaarVerification: !prev.requireAadhaarVerification }))}
              className={`w-12 h-6 rounded-full relative flex items-center px-0.5 transition-colors ${form.requireAadhaarVerification ? 'bg-emerald-500' : 'bg-slate-700'}`}
            >
              <span className={`w-5 h-5 rounded-full bg-slate-950 transition-transform ${form.requireAadhaarVerification ? 'translate-x-6' : 'translate-x-0'}`} />
            </button>
          </div>
          <div>
            <label className={labelClass}>Replace Banner Image (optional)</label>
            <input type="file" accept="image/*" onChange={(e) => setBannerFile(e.target.files[0])} className="w-full text-xs text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-slate-200 hover:file:bg-slate-700" />
          </div>
        </section>

        {/* Actions */}
        <div className="flex items-center justify-end gap-4 pt-4 border-t border-slate-800">
          <button type="button" onClick={() => navigate('/admin/tournaments')} className="px-5 py-2.5 text-xs font-semibold text-slate-400 hover:text-white">Cancel</button>
          <button type="submit" disabled={saving} className="px-8 py-3 rounded-2xl font-bold text-xs uppercase tracking-wider bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-lg shadow-amber-500/20 disabled:opacity-50 transition-all">

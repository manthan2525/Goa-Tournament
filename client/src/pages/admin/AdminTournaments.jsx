...
const AdminTournaments = () => {
  ...
  const statusStyle = (s) => {
    const map = {
      ...
      ONGOING: 'bg-rose-500/15 text-rose-400',
      COMPLETED: 'bg-slate-500/15 text-slate-400',
      REGISTRATION_CLOSED: 'bg-amber-500/15 text-amber-400',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-black text-2xl text-white flex items-center gap-2">
            <Trophy className="w-6 h-6 text-amber-400" /> Tournament Management
          </h1>
          <p className="text-xs text-slate-400 mt-1">{pagination.total} tournaments across all organizers</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500"
          />
        ...
        <select
          ...
          className="px-3 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-amber-500"
        >
      ...
      {successMsg && <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300">{successMsg}</div>}
      ...
      <div className="rounded-2xl glass-card border border-slate-800 overflow-hidden">
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                ...
                <tr><td colSpan="9" className="px-5 py-16 text-center text-slate-500">No tournaments found.</td></tr>
              ) : (
                tournaments.map((t) => (
                  <tr key={t._id} className="hover:bg-slate-800/40 transition">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-amber-500/10 border border-amber-500/20 overflow-hidden flex-shrink-0 flex items-center justify-center">
                          {t.bannerImage ? (
                            <img src={t.bannerImage} alt={t.name} className="w-full h-full object-cover" />
                          ) : <Trophy className="w-3.5 h-3.5 text-amber-400" />}
                        </div>
                        <span className="font-semibold text-white max-w-[160px] truncate">{t.name}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/tournaments/${t._id}`} title="View" className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition">
                          <Eye className="w-3.5 h-3.5" />
                        </Link>
                        <Link to={`/admin/tournaments/${t._id}/edit`} title="Edit" className="p-1.5 rounded-lg text-amber-400 hover:bg-amber-500/10 transition">
                          <Edit className="w-3.5 h-3.5" />
                        </Link>
                        <button title="Delete" onClick={() => setDeleteModal(t)} className="p-1.5 rounded-lg text-rose-400 hover:bg-rose-500/10 transition">
        ...
        {pagination.pages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-800">
            <p className="text-[10px] text-slate-500">Page {page} of {pagination.pages} ({pagination.total} total)</p>
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-md rounded-2xl glass-panel border border-rose-500/30 p-6 space-y-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-rose-400 flex-shrink-0 mt-0.5" />
              <div>
                <h3 className="font-bold text-white text-sm">Delete Tournament?</h3>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setDeleteModal(null)} className="flex-1 px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-400 bg-slate-800 hover:bg-slate-700 transition">Cancel</button>

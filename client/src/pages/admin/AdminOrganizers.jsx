...
const ConfirmModal = ({ title, description, onConfirm, onCancel, confirmLabel = 'Confirm' }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
    <div className="w-full max-w-sm rounded-2xl glass-panel border border-slate-700 p-6 space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={onCancel} className="flex-1 px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-400 bg-slate-800 hover:bg-slate-700 transition">Cancel</button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-black text-2xl text-white flex items-center gap-2">
            <Building2 className="w-6 h-6 text-violet-400" /> Organizer Management
          </h1>
          <p className="text-xs text-slate-400 mt-1">{pagination.total} organizers on the platform</p>
        </div>
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-violet-500"
          />
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i}>{Array.from({ length: 7 }).map((_, j) => (
                    <td key={j} className="px-5 py-3"><div className="h-3 w-24 bg-slate-800 rounded animate-pulse" /></td>
                  ))}</tr>
                ))
              ) : organizers.length === 0 ? (
                <tr><td colSpan="7" className="px-5 py-16 text-center text-slate-500">No organizers found.</td></tr>
              ) : (
                organizers.map((org) => (
                  <tr key={org._id} className="hover:bg-slate-800/40 transition">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-violet-500/20 border border-violet-500/30 flex items-center justify-center text-xs font-bold text-violet-400 overflow-hidden flex-shrink-0">
                          {org.profilePhoto ? <img src={org.profilePhoto} alt={org.name} className="w-full h-full object-cover" /> : org.name?.charAt(0)}
                        </div>
                        <span className="font-semibold text-white whitespace-nowrap">{org.name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-slate-400 whitespace-nowrap">{org.email}</td>
                    <td className="px-5 py-3 text-slate-400">{org.organizationName || '—'}</td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase font-mono ${org.isActive !== false ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/admin/tournaments?organizer=${org._id}`} title="View tournaments" className="p-1.5 rounded-lg text-violet-400 hover:bg-violet-500/10 transition">
                          <Trophy className="w-3.5 h-3.5" />
                        </Link>
                        <button title="Deactivate organizer" onClick={() => setDeleteModal(org)} className="p-1.5 rounded-lg text-rose-400 hover:bg-rose-500/10 transition">

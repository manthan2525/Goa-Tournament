...
const AdminLiveScores = () => {
  ...
  // Handle Delete Match / Live Score
  const handleDeleteMatch = async () => {
    if (!deleteModal) return;
    try {
      setDeleteLoading(true);
      await api.delete(`/admin/matches/${deleteModal._id}`);
      setSuccessMsg('Live score / match successfully deleted.');
      setDeleteModal(null);
      fetchMatches();
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err) {
      setError(err.message || 'Failed to delete live score / match.');
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
            Real-time score update, fixture editing, and live match deletion for Admin.
          </p>
        </div>
